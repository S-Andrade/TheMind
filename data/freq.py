...
while (i < video_size):
    ...
    if player0[i] == 'look-at-player1' and player1[i] == 'look-at-player0':
        if gameR[index].p0p1[0] == True:
            gameR[index].p0p1[1][-1] +=1
            p0p1[1][-1] +=1
        if gameR[index].p0p1[0] == False:
            gameR[index].p0p1[1].append(1)
            p0p1[1].append(1)
            gameR[index].p0p1[0] = True
            p0p1[0] = True
    if player0[i] == 'look-at-player2' and player2[i] == 'look-at-player0':
        if gameR[index].p0p2[0] == True:
            gameR[index].p0p2[1][-1] +=1
            p0p2[1][-1] +=1
        if gameR[index].p0p2[0] == False:
            gameR[index].p0p2[1].append(1)
            p0p2[1].append(1)
            gameR[index].p0p2[0] = True
            p0p2[0] = True
    if player1[i] == 'look-at-player2' and player2[i] == 'look-at-player1':
        if gameR[index].p1p2[0] == True:
            gameR[index].p1p2[1][-1] +=1
            p1p2[1][-1] +=1
        if gameR[index].p1p2[0] == False:
            gameR[index].p1p2[1].append(1)
            p1p2[1].append(1)
            gameR[index].p1p2[0] = True
            p1p2[0] = True
...
with open('freq.txt', 'w') as file:
    for g in gameR:
        ...
        if len(g.p0p1[1]) > 0:
            file.write("p0p1\n")
            printNice("Freqencia" , sum(g.p0p1[1])/g.size*100)
        ...
        if len(g.p0p2[1]) > 0:
            file.write("p0p2\n")
            printNice("Freqencia" , sum(g.p0p2[1])/g.size*100)
        ...
        if len(g.p1p2[1]) > 0:
            file.write("p1p2\n")
            printNice("Freqencia" , sum(g.p1p2[1])/g.size*100)
        ...
        if len(total) > 0:
            file.write("Total\n")
            printNice("Freqencia" , sum(total)/video_size*100)
    ...
    file.write("p0p1\n")
    printNice("Freqencia" , sum(p0p1[1])/video_size*100)
    printNice("Media" , sum(p0p1[1])/len(p0p1[1])/1000)
    printNice("Max" , max(p0p1[1])/1000)
    printNice("Min" , min(p0p1[1])/1000)

    file.write("p0p2\n")
    printNice("Freqencia" , sum(p0p2[1])/video_size*100)
    printNice("Media" , sum(p0p2[1])/len(p0p2[1])/1000)
    printNice("Max" , max(p0p2[1])/1000)
    printNice("Min" , min(p0p2[1])/1000)

    file.write("p1p2\n")
    printNice("Freqencia" , sum(p1p2[1])/video_size*100)
    printNice("Media" , sum(p1p2[1])/len(p1p2[1])/1000)
    printNice("Max" , max(p1p2[1])/1000)
    printNice("Min" , min(p1p2[1])/1000)

    total = p0p1[1] + p0p2[1] + p1p2[1]
